...
fn code_from_str(code_str: &str) -> CodeState
{
    CodeState
    {
...
{
    let mut inned = true;
    //let code = &mut code_state.code;
    //let i = &mut code_state.inst_ptr;
    //let rel_base = &mut code_state.rel_base;
    ...
    while cs.inst_ptr < cs.code.len()
    {
        ...
        /*
        let opcode = match Opcode::try_from(val % 100)
        {
            Ok(x)  => x,
            Err(e) => { println!("Invalid opcode! {}", e.to_string()); *i+1; break; }
        };*/
        val /= 100;
        ...
        match opcode
        {
            ...
            Opcode::Input       => if inned { cs.code[a] = input; inned = false; } else { break; },
