...
fn part_one()
{
    ...
    for y in (CENTRE-40..CENTRE+30).rev()
    {
        for x in CENTRE-10..CENTRE+130
        {
            if panel[x][y]
            {
                print!("*");
    ...
    let mut min_dist:usize = 999999;

    x = CENTRE;
    y = CENTRE;
    for mv in input2.split(",")
    {
        let dist:u16 = mv[1..].parse().unwrap();
        //println!("{}: {}", mv, dist);
        for _ in 1..=dist
        {
            ...
            if panel[x][y]
            {
                let x0: isize = x as isize - CENTRE as isize;
                let y0: isize = y as isize - CENTRE as isize;
                println!("Found intersection at ({},{})", x0, y0);
                let man_dist:usize = (x0.abs() + y0.abs()) as usize;
                if man_dist < min_dist
                {
                    min_dist = man_dist;
                }
                println!("Min dist is now {}.", min_dist);
            }
        }
    }
...
*/
    //Now iterate through the second wire's movements, finding and recording intersections.
    ...
    for mv in input2.split(",")
    {
        ...
        for _ in 1..=dist
        {
            ...
            if panel[x][y]
            {
                let x0: isize = x as isize - CENTRE as isize;
                let y0: isize = y as isize - CENTRE as isize;
                hits.push(Hit(x0, y0, travelled));
                
                println!("Found intersection at ({},{}), having travelled {}.", x0, y0, travelled);
            }
        }
    }

    println!("");
    println!("Found total of {} intersections. Now to calc travel distances.", hits.len());
    println!("");

    //Finally, iterate back through the first wire's movements, recording distance to intersections.
    let mut min_trav:usize = 999999;
    travelled = 0;
    x = CENTRE;
    y = CENTRE;
    ...
    for mv in input1.split(",")
    {
        ...
        for _ in 1..=dist
        {
            ...
            for hit in &hits
            {
                ...
                if x0 == hit.0 && y0 == hit.1
                {
                    let this_trav:usize = travelled + hit.2;
                    if this_trav < min_trav
                    {
                        min_trav = this_trav;
                    }
                    println!("After intersection ({},{}), min trav is now {}.", x0, y0, min_trav);
                }
            }
        }
    }
